...
5. **Доступ к сервисам:**
   - 🌐 **Админка:** http://localhost:45554 (логин: admin123)
   - 🤖 **Бот API:** работает автоматически
   - 📊 **API документация:** http://localhost:45556/docs
...
**В Docker:** Админка автоматически доступна на http://localhost:45554
**Локально:** Запустите `cd frontend && npm start` (порт 3000 по умолчанию)
...
- Фильтры по статусу
- Просмотр деталей заказа (с автоматическим разрешением путей к фото)
- Изменение статуса (с автоматическим уведомлением клиента в Telegram)
...
1. **Авторизация:** `/iamadmin BOT_ADMIN_PASSWORD` (берётся из .env)
...
*   **Логи:** Используйте `docker compose logs -f` для отладки.
*   **База данных:** Структура хранится в `schema.sql` и разворачивается автоматически.
*   **Скрипты:** `docker-entrypoint.sh` управляет запуском сервисов в контейнерах.
